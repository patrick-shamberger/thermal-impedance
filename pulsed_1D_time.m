% Script 'pulsed_1D_time.m' v1.0.0, tested 28 September 2024
% Written by P.J. Shamberger (c) 2023

%% PULSED_1D_TIME ---------------------------------------------------------
% Finite Difference Scheme: 1D model, periodic boundary conditions
%
% pulsed_1D_time.m is a finite difference scheme that performs finite 
...
% MODEL VARIABLES (start) -------------------------------------------------
    ...
    % TIMING ..............................................................
        ...
        n_cycles = timing.n_cycles;              % total number of cycles
        n_cycles_min = timing.n_cycles_min;      % minimum # of cycles
              
        n_oscill_test = timing.n_oscill_test;      % Melt interface oscillations over last n cycle (note: n_cycles_min > n_oscill_test)
    ...
    % GEOMETRY ............................................................
        ...
        H_bond_top = geometry.H_bond_top;   % [m] thickness top interface: thermal epoxy/Al foil/thermal epoxy
        H_slab = geometry.H_slab;           % [m] slab thickness
        H_bond_bot = geometry.H_bond_bot;   % [m] thickness bottom interface: thermal paste/Al foil/thermal epoxy
...
% 1 = Cu foam + PCM
% 2 = Cu foam (empty)
% 3 = bond_top property (thermal epoxy/Al foil/thermal epoxy)
% 4 = bond_bot property (thermal epoxy/Al foil/thermal paste)
...
Lv_comp = Mat(PCM,5).*Vfp;  % Volumetric Latent Heat of Fusion in j/m^3 of the PCM
...
A = zeros(N);
B = zeros(N,1);
hft = zeros(N,1);  % defines as initially all solid.
sl = zeros(N,1);
T = ones(N,1)*Tinf;
%T = Tm;

if input_Temp_flag
    % use the "average power" to determine an initial temperature distribution.
    T = T_input;

    % based on this "average power", set the initial state of PCM elements to be fully melted IF T_input > Tm
    hft(find(P==2 & T_input>Tm)) = Lw(2);
...
h_waitbar = waitbar(cycle_index./n_cycles,'Running Pulse Train...');
...
for ii = 1:1:(length(time1)-1) %step in time
    ...
    for i = 1:1:m %step in y
        ...
        % -----------------------------------------------------
        % TOP SURFACE (Heated surface): Heating BC (convec, T, q" BC's, as specified)
        if i == 1   
            % MODIFIED
            K4_eff = 2.*K(i).*K(i+1)./(K(i) + K(i+1));
        ...
        % -----------------------------------------------------
        % BOTTOM SURFACE (HS surface): Adiabatic OR set to constant cooling (convec, T, q" BC's, as specified)
        if i == m
            % MODIFIED
            K3_eff = 2.*K(i).*K(i-1)./(K(i) + K(i-1));
    ...
    if convergence_flag == 1
        if mod(ii,pts_per_cycle)==0
            ...
            waitbar(cycle_index./n_cycles, h_waitbar,'Running Pulse Train...');
            ...
            if cycle_index > n_cycles_min   % only check for convergence after n_cycles_min cycles
                ...
                if test_value < convergence_delta
                    ...
                    T_hot(ii+1:end) = [];
                    qtot2(ii+1:end) = [];
                    fprintf('Convergence achieved. Cycle index: %f. Time loop terminated.\n', cycle_index);

                    break
                elseif max1 > 1000  % temperature 
                    ...
                    disp('temperature diverging. decrease convection coefficient or timesteps. time loop terminated');
                    break
                end
            end
        end
...
for i=1:n_oscill
    max_Y(i) = max(Ymelt((endpoint - pts_per_cycle*i):(endpoint - pts_per_cycle*(i-1))));
    min_Y(i) = min(Ymelt((endpoint - pts_per_cycle*i):(endpoint - pts_per_cycle*(i-1))));
...
if ~BC_flag      % selects amplitude of periodic q" BC
    for i=1:n_oscill
        max_T_hot(i) = max(T_hot((endpoint - pts_per_cycle*i):(endpoint - pts_per_cycle*(i-1))));
        %cycle_latent_sum(i) = sum(qlatent((endpoint - pts_per_cycle*i):(endpoint - pts_per_cycle*(i-1))));
        %cycle_sensible_sum(i) = sum(qsense((endpoint - pts_per_cycle*i):(endpoint - pts_per_cycle*(i-1))));

        % how much heat goes into melting the PCM each cycle?
        cycle_latent(i) = sum(abs(qlatent((endpoint - pts_per_cycle*i):(endpoint - pts_per_cycle*(i-1))))).*dt_step/2;  % J/m2
    ...
    % how much heat goes into melting the PCM each cycle?
    cycle_latent_out = mean(cycle_latent);
    % what fraction of PCM latent heat is utilized?
    utilization_out = cycle_latent_out./(Lv_comp.*H_slab);
    % how much heat goes into melting the PCM each cycle?
    storage_fraction_out = cycle_latent_out./(q_amplitude.*t1);
